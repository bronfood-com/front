    ...
    return (
        <ul
            className={`${styles.filter__options} ${
                !selected && styles.filter__options_nowrap
            }`}
        >
            {options.map((option) => (
                <li key={option.id}>
...
const Filter = ({ close }: { close: () => void }) => {
    ...
    const suggestedOptions: Option[] = input
        ? options.all.filter(
              (opt) =>
        ...
        <div className={styles.filter_overlay} onClick={handleOverlayClick}>
            <div className={styles.filter}>
                <div className={styles.filter__title_container}>
                    <button
                        className={styles.filter__icon_back}
                        type="button"
                        onClick={close}
                    />
                    <p
                        className={`${styles.filter__text} ${styles.filter__text_bold}`}
                    >
                        {t('pages.filter.filters')}
                    </p>
                </div>
                    ...
                    <div className={styles.filter__options_container}>
                        <div className={styles.filter__input_container}>
                            ...
                            <input
                                ...
                                type="text"
                                placeholder={t('pages.filter.placeholder')}
                            />
                        </div>
                        <div className={styles.filter__options_list}>
                            <OptionList
                                ...
                                selected={false}
                                action={options.addOption}
                            />
                            <OptionList
                                options={options.selectedOptions}
                                selected={true}
                                action={options.deleteOption}
                            />
                        </div>
                    </div>
                </div>
                <div className={styles.filter__search_container}>
                    ...
                    <ul className={`${styles.filter__types}`}>
                        {venueTypes.all.map((type) => {
                            const isActive =
                                venueTypes.selectedVenueTypes.includes(type);
                            return (
                                <li key={type.id}>
                                    <Chip
                                        text={t(`pages.filter.${type.name}`)}
                                        isActive={isActive}
                                        add={() =>
                                            venueTypes.addVenueType(type)
                                        }
                                        delete={() =>
                                            venueTypes.deleteVenueType(type)
                                        }
                                    />
                                </li>
                            );
